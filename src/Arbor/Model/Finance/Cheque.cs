...
namespace Arbor.Model.Finance
{
    ...
    {
        protected string resourceType = ResourceType.FINANCE_CHEQUE;
        ...
        public static ModelCollection<Cheque> query (SimpleQuery query = null)
        {
        	...
        	query.setResourceType ("Finance_Cheque");
        ...
        public static Cheque retrieve (string id)
        {
            RestGateway gateway = (RestGateway) Cheque.getDefaultGateway();
        	if(gateway == null) throw new Exception("You must call ModelBase::setDefaultGateway() prior to calling query()");
        	return (Cheque) gateway.retrieve(ResourceType.FINANCE_CHEQUE, id);
        }
