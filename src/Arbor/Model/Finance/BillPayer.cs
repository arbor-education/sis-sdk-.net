...
namespace Arbor.Model.Finance
{
    ...
    {
        protected string resourceType = ResourceType.FINANCE_BILL_PAYER;
        ...
        public static ModelCollection<BillPayer> query (SimpleQuery query = null)
        {
        	...
        	query.setResourceType ("Finance_BillPayer");
        ...
        public static BillPayer retrieve (string id)
        {
            RestGateway gateway = (RestGateway) BillPayer.getDefaultGateway();
        	if(gateway == null) throw new Exception("You must call ModelBase::setDefaultGateway() prior to calling query()");
        	return (BillPayer) gateway.retrieve(ResourceType.FINANCE_BILL_PAYER, id);
        }
