...
{
    public class UkDfe_ChildCarePlace : ModelBase
    {
        ...
        public UkDfe_ChildCarePlace ()
        {
            base.resourceType = this.resourceType;
        }
        
        public UkDfe_ChildCarePlace (string resourceType = "UkDfe_ChildCarePlace", Hashtable properties = null, IGateway apiGateway = null) 
        ...
        public static ModelCollection<UkDfe_ChildCarePlace> query (SimpleQuery query = null)
        {
            if (query == null) query = new SimpleQuery ();
        	query.setResourceType ("UkDfe_ChildCarePlace");
        	RestGateway gateway = (RestGateway) UkDfe_ChildCarePlace.getDefaultGateway ();
        	if(gateway == null) throw new Exception("You must call ModelBase.setDefaultGateway() prior to calling query()");
        
        	ModelCollection<UkDfe_ChildCarePlace> ukdfe_childcareplaceCollection = new ModelCollection<UkDfe_ChildCarePlace> ();
        	ModelCollection<ModelBase> collection = gateway.query (query);
        
        	foreach (ModelBase model in collection) {
        	    ukdfe_childcareplaceCollection.add((UkDfe_ChildCarePlace) model);
        	}
        
        	return ukdfe_childcareplaceCollection;
        }

        public static UkDfe_ChildCarePlace retrieve (string id)
        {
            RestGateway gateway = (RestGateway) UkDfe_ChildCarePlace.getDefaultGateway();
        	if(gateway == null) throw new Exception("You must call ModelBase::setDefaultGateway() prior to calling query()");
        	return (UkDfe_ChildCarePlace) gateway.retrieve(ResourceType.UK_DFE_CHILD_CARE_PLACE, id);
        }
