...
{
    public class UkDfe_SchoolCensusReturn : ModelBase
    {
        ...
        public UkDfe_SchoolCensusReturn (string resourceType = "UkDfe_SchoolCensusReturn", Hashtable properties = null, IGateway apiGateway = null) 
        ...
        public static ModelCollection<UkDfe_SchoolCensusReturn> query (SimpleQuery query = null)
        {
            if (query == null) query = new SimpleQuery ();
        	query.setResourceType ("UkDfe_SchoolCensusReturn");
        	RestGateway gateway = (RestGateway) UkDfe_SchoolCensusReturn.getDefaultGateway ();
        	if(gateway == null) throw new Exception("You must call ModelBase.setDefaultGateway() prior to calling query()");
        
        	ModelCollection<UkDfe_SchoolCensusReturn> ukdfe_schoolcensusreturnCollection = new ModelCollection<UkDfe_SchoolCensusReturn> ();
        	ModelCollection<ModelBase> collection = gateway.query (query);
        
        	foreach (ModelBase model in collection) {
        	    ukdfe_schoolcensusreturnCollection.add((UkDfe_SchoolCensusReturn) model);
        	}
        
        	return ukdfe_schoolcensusreturnCollection;
        }

        public static UkDfe_SchoolCensusReturn retrieve (string id)
        {
            RestGateway gateway = (RestGateway) UkDfe_SchoolCensusReturn.getDefaultGateway();
        	if(gateway == null) throw new Exception("You must call ModelBase::setDefaultGateway() prior to calling query()");
        	return (UkDfe_SchoolCensusReturn) gateway.retrieve(ResourceType.UK_DFE_SCHOOL_CENSUS_RETURN, id);
        }
